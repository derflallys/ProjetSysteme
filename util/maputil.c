...
void createCopy(char * filesrc,char * filedest,int position,char * option)
{
    ...
    if(position==0)
    {
        ...
        for (int i = 0; i <nbelmts ; ++i) {
            ...
            if(w==-1)
            {
                perror("Erreur de ecriture du fichier obj");
                exit(0);
            }
            if(w==0)
            {
    ...
    for(int i= 0 ; i<nbobjects;i++)
    {
        ...
        if (r == -1) {
            perror("Erreur de lecture frame \n");
            exit(0);
        }
        if (r == 0) {
            perror("Zero caractere lu \n");
            ...
            if(width<hORw)
            {
                ...
                if(fd!=-1)
                {
                    ...
                    if(fdBackup!=-1)
                    {
                        ...
                        int pos = 4*sizeof(int)+newnbelemts*(3*sizeof(int));
